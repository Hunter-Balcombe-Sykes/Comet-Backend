...
-- rather than assumed.
--
-- This WIDENS the domain: every existing row satisfies the new set by
-- construction, so no NOT VALID / VALIDATE split is needed (§2 exists for
-- constraints that could fail an existing-row scan; a superset cannot). The
-- table also holds 0 rows on dev, so the ACCESS EXCLUSIVE window is nil.
--
-- ROLLBACK: BEGIN;
...
--                 'bio_harvest','google_business','staff','reproject']));
--           COMMIT;
--           (Reverting re-breaks CommerceProbeJob, and fails outright if any
--            'commerce_probe' row has landed by then — DELETE those first.)

BEGIN;

-- IF EXISTS so a re-apply after ledger drift is a no-op rather than a hard
-- failure (the MCP stamps its own version, so db push may re-run this file).
