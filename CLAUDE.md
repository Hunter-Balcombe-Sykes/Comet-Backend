...
**Current reality (2026-07-26, post-cutover):** Each env now stands on its own. **Production** serves `api.partna.au` from the prod Laravel Cloud env backed by prod Supabase (`edplucmvkcnokyygxqsb`); **development** serves `dev-api.partna.au` from dev Supabase (`glncumufgaqcmqhzwrxm`). They deploy independently — pushing `development` no longer updates prod. To deploy prod: `git push origin development:production` — the prod env has `usesPushToDeploy: true`, so **the push IS the deploy** (no promote step, no approval, and **no CI gate** — CI's real gate runs on `development`). Apply migrations against the ref you mean. **Prod schema has DIVERGED from dev** — dev has taken many migrations since the 2026-07-26 baseline that were never applied to prod; prod reconciliation is deferred, separate future work (do not assume parity, verify against the ref). Prod carries no customer data yet (`core.users` = 0). Supabase org is on the **Pro** plan (upgraded 2026-08-14) — daily managed backups exist; the R2 dump remains the belt-and-braces backup. **GitHub org (moved 2026-09-04):** both repos live in **`PartnaAu`** — `PartnaAu/partna-backend` (**public**, deliberately: public repos get unlimited Actions minutes and branch protection free; on PartnaAu's free plan, going private would meter Actions AND silently drop the 9 required checks) and `PartnaAu/partna-db-backup` (private). They were in `Hunter-Balcombe-Sykes`; GitHub redirects the old URLs indefinitely, so stale references fail silently rather than loudly. Laravel Cloud binds the repo on the **application** (`repositoryFullName`), not per-environment — `environment:get` has no repository field, and each env's `branch` is unrelated. Runbook + the transfer's observed gotchas: `docs/deploy/github-org-transfer.md`. **Two different names, do not conflate them:** `partna-db-backup` (singular) is the GitHub REPO holding the backup workflows; the R2 BUCKET is `partna-db-backups` (plural). `scripts/db/backup-to-r2.sh` reaches it from a laptop — not via `AWS_*`/`R2_*` keys (there are none locally; the scoped ones are GitHub Actions secrets) but via the existing wrangler OAuth session, driven through `npx --yes wrangler@4`, so nothing needs installing.
...
- One LIVE build per source for the OUTREACH lanes only (`pre_account_builds_live_source_unique`, partial on `claimed_at IS NULL AND built_via <> 'signup'` since 2026-09-03). **Signup builds multiply freely** (owner call): the same IG/GBP source may start any number of independent sign-ups — `requestBuild` never dedupes or re-serves for the signup lane, and `findLive` excludes `built_via='signup'` so staff/ManyChat can never re-serve (or attach an address/token to) a self-serve build. Failed OUTREACH builds reset + re-run on dedupe hit; a failed signup build is just left behind and the next signup builds fresh.
...
- **CI: nine required checks run on EVERY commit, but a prose-only diff skips their expensive steps** (`e342285cb`, 2026-09-05). A `changes` job classifies the diff (~6s) and the eight code-driven jobs gate their steps on `needs.changes.outputs.prose_only`; `test` alone is ~20 min, so a `CLAUDE.md` edit used to cost that for nothing. Three things NOT to "tidy": (1) **`supply-chain` is deliberately ungated** — gitleaks reads `docs/` and `*.md`, and a key pasted into a runbook is an observed vector; (2) **prose means `docs/**` and root-level `*.md` ONLY** — `audits/**` and `scripts/audit/**/*.md` are CODE, because `AuditPipelineIntegrityTest` reads `base_path('audits')` and globs `lenses/*.md`; (3) **every gated job's `if:` must keep `!cancelled()`** — with the implicit `success()`, a failed `changes` job SKIPS eight required checks, which is the never-reported-check failure the design exists to avoid. `paths:` filters remain refused for that same reason. Detector: `.github/scripts/prose-only-diff.sh` (fails closed — unreadable diff, force push or new branch all mean "run everything"). Pinned by `CiTestGateIntegrityTest`.
...
**A pass-composing path resolves pools through the BATCHED seam, never per pool.** `SetupPayload` plans each pool, unions the ids, hydrates ONCE (`PoolResolver::preloadSections`/`preloadCuration`/`plan`/`hydrateItems`/`assemble`) and assembles per pool — the same seam `PoolWire::forSite` uses. Measured on dev 2026-09-05: `GET /api/site/setup` p50 765ms -> 569ms, p90 1502ms -> 964ms. A new call site that loops `PoolResolver::resolve()` per pool silently reinstates the N+1. Two things that look like tidying and are not: the hydrate keeps `withDuplicateCandidates: true` (the `items.*`/`media`/`links` passes put library rows on the wire VERBATIM, so `duplicateCandidates` IS public there — pinned by `tests/Feature/Setup/SetupPoolBatchingTest.php`), and `forPass()` preloads sections for EVERY pool while hydrating only its own, because `preloadSections` provisions missing `site.sections` rows as a side effect.
...
DIY k6 harness against dev only (README + plan: `docs/superpowers/plans/2026-07-26-k6-load-testing.md`). **`seed.sql` was rebuilt 2026-09-02 for the content lane** — it had been raising `42P01` on the dropped `site.services` and seeding a retired `site_media` pool, so the harness could not run at all. It now mints the real `content.*` shapes and the run was verified end-to-end against dev-api (`pools.media`=6, `pools.services`=15, `pools.custom_links`=10). Its `seed.sql`/`jobs.js` hard-code real invariants that have silently broken them twice — touching any of these, re-check the harness:
- **Two pools publish by DIFFERENT mechanisms.** `services`/`custom_links` take the `kind_is` rule and publish automatically; `media` is opt-in (`kind_is` + `latest_n_per_auto_source`) and a MANUAL source is not an auto source — so media items publish only when **pinned** via `site.section_items`. Seeding media without pins yields `pools.media.items == []` and a silently empty page.
- ~~Gallery capped at 6/site~~ **RETIRED 2026-09-02.** The `gallery` pool and its `core.enforce_site_gallery_max6` trigger are both dropped (migration `20260902170000`); the seed uses the `content` pool, capped at 20 by `config('partna.image_pools.content.max')`. The 6 it seeds is a historical constant so past baseline results stay comparable, not a ceiling.
- A media item needs a matching `site.media_variants` (webp) row or its URL resolves empty — guarded by `tests/Feature/Api/PublicSite/IndividualProfileControllerTest.php`'s gallery-engine tests. The seed links each `content.media_assets` row to its `site_media_id`, which is the bridge the real upload lane writes.
- **`profile.services`, `profile.links` and `profile.gallery` are all GONE from the wire.** Curated content is at `data.profile.pools.<pool>.items`. `baseline.js` asserted the three dead keys until 2026-09-02 — `undefined` is not an Array, so the check returned false on every request and the run passed anyway, because a k6 `check` returning false is recorded, not fatal.
