...
Hard rules:
- `'unclaimed'` is first-class, but **"Dark Until Claimed" (2026-08-24) gates it first, and that gate is NOT `is_published`.** An unclaimed build is publicly routable only if someone vetted it — `PreAccountBuild::isVisibleWhileUnclaimed()`: staff-built, or a staff-approved early-access lead with an `expires_at`. An unvetted self-serve (`built_via='signup'`) build 404s at `IndividualProfileController::show` AND `PublicIntegrationController::show`, and `SyncSubdomainToKvJob` never writes its KV entry (it retires an existing one). **The old "a pre-account site is public pre-claim by design" rule is dead for self-serve builds — do not design against it.** Past that gate, `is_published` is a separate, orthogonal flag the profiles route and KV write still ignore (KV TTL from `pre_account_builds.expires_at`). Capability/notification/deletion gates fail-closed. Note `isVisibleWhileUnclaimed()` is deliberately NARROWER than `isOutreach()` (which also covers `VIA_STAFF`/`VIA_EARLY_ACCESS`), so every publicly-visible unclaimed build is email-gated at claim time and the first-come arm only ever sees dark builds. Gating log: `tests/Feature/PreAccount/UnclaimedGatingTest.php`; `docs/api.md` §Public visibility vs. `is_published`.
