...
**Push to Supabase:** `supabase link --project-ref <ref>` → `db push --dry-run` → `db push`. Dev freely; prod confirm first. Fresh prod DB: `ALTER ROLE app_backend WITH LOGIN PASSWORD '<secret>'` (NOLOGIN by default). `DB_USERNAME` = `app_backend.<project_ref>` (Supavisor), port 5432.

## Tech Stack

PHP 8.2 / Laravel 12 / PostgreSQL (Supabase: schemas `public`, `core`, `site`, `notifications`, `analytics`, `audit`) / Supabase Auth (JWT) / Redis / Laravel Horizon / Pest 4 + PHPUnit (SQLite in-memory) / Laravel Nightwatch.
...
Full spec: `../docs/superpowers/specs/2026-05-26-skeleton-system-design.md`.
