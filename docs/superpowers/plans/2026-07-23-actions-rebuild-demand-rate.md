...
- [x] Steps: failing test → implement → `npm run typecheck && npm run lint` + test PASS → doctrine self-check on the diff → ~~live-verify in browser~~ → commit `feat(design): manual action-order dialog behind smart toggle`.
  - **Caveat (2026-07-24):** typecheck/lint/full test suite (114 files, 623 tests) all green, including 3 dedicated `action-order-dialog.test.tsx` cases (seed-from-ranked, seed-from-manual-with-append, save-payload-shape). The actual "toggle off → row appears → drag → save → GET /site/actions reflects" click-through in a real logged-in browser was NOT completed — it requires entering the test account's password, which is outside what Claude may do autonomously (hard rule, not a judgment call), and no session was already active. Deferred to whoever is present to log in — either the user manually confirms this flow once, or hands Claude an already-authenticated session to continue from.
  - A real render-loop bug was caught and fixed during this step (`data?.pool ?? []` inline in a `useMemo`/`useEffect` dependency chain created a fresh array reference every render, causing an infinite loop that pinned a test process at ~300% CPU) — fixed by memoizing on the stable `data` reference itself instead of its `??`-defaulted sub-fields. Worth remembering as a general footgun, not just local to this file.
  - A fresh-eyes review agent (dispatched specifically for this task) independently reproduced the same render-loop bug via its own scratch repro AND caught a second, more serious one: the dialog only shows/drags pool actions, so on save it rebuilt `manual_actions` purely from the dragged pool order — silently and permanently dropping any pre-existing `{kind:"custom",label,url}` entry (ad hoc buttons with no pool id, which `SiteActionsService::resolveRankedActions()`'s manual-mode branch still renders live, independent of the pool). Fixed: these are now preserved verbatim through every save (not shown/draggable, but never lost), with a dedicated regression test. Also wired `setActivatorNodeRef` (dnd-kit best practice for a handle separate from the sortable root — was harmless today, fragile going forward).

**Phase 3 gate:** push `main` → Vercel; flip smart off on the test account, reorder, confirm ollies payload `ranked_actions` obeys manual order end-to-end. **Blocked on the same login constraint above, plus the push itself (deploy-triggering, deferred to the user per the established autonomous-session safety boundary — see Phase 4 below).**
