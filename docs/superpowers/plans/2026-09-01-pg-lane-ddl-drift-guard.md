...
- **No test was found to pass only because a column was missing.** All 10 work-queue entries were genuine drift on stand-ins that already exercised the reading code correctly — the missing column made the *guard* fail (SQLSTATE 42703 or a static finding), never made an assertion pass that should have failed. Nothing here was silently correct-by-accident.
- **The guard found drift the hand-written fix in PR #316 missed.** That PR patched `da958493e`'s `rs.last_seen_run` regression by hand across the five red stand-ins, but two more instances of the same bug class were still sitting there uncaught: `content.media_assets.mirror_eligible` absent from four projection stand-ins (see below — this is the same finding, restated with its guard-run provenance), and `ingest.record_state.current_version_id` still missing from `IngestProjectChunkingTest` after PR #316 had already opened that exact file for a different column (`last_seen_run`) and closed it again without noticing the sibling gap. Both are exactly the failure mode this guard exists to catch statically instead of by hand.
- **Two findings are recorded in the guard's `$knownLimitation` rather than fixed.** `content.items.removed_at`/`updated_at` on `ShopCatalogueCreatedAtTimezoneTest` are a genuine drift signal by the guard's own logic, but not real drift on inspection: they are an artefact of per-class (not per-method) attribution — `ShopContentWriter::retireStaleItems()` writes both columns, but the test only calls `cataloguesFor()`, which never touches either. Thinning the app-side scanner to method granularity was judged out of scope for this guard; the entry stays in `$knownLimitation`, not `$exempt`, because it is a per-run reproducible finding rather than a whole-table skip, and its own staleness check (`$stale`/`array_diff`) means it cannot linger silently if `cataloguesFor()` ever starts reading either column for real.
- **`AppColumnReadScanner` has a third accepted miss, undocumented until the whole-branch review caught it: the `->from('schema.table')` closure/subquery idiom.** `CHAIN_START` anchors only on `DB::table(`/`->table(`, deliberately not `->from(` — a query built as `$e->from('content.f_occurrence')` inside a `whereExists`/`whereIn` closure is invisible unless it happens to fall inside an outer chain's span, and adding `->from()` as a second chain start would let two spans overlap the same text and corrupt attribution between them. Measured 2026-09-01: 8 app/ classes have at least one such call outside every chain span, reaching 12 tables only this way. Live cost: `SectionOccurrenceOrderingTest` provisions `content.source_items` and `content.f_occurrence`, and both go unchecked, because `SectionCandidates` reaches them only through this idiom (`app/Site/Sections/SectionCandidates.php:250-256, 340`). Fails safe, same as the other two known misses — a hidden gap, never a false alarm — so it was documented rather than fixed.
