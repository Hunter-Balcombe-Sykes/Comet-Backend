...
- [x] **Step 4: Repoint the Tickets & Events card's add-an-event verb** — `EventsCatalog::storeStandalone()` now writes the pool through `ManualEventWriter::addStandalone()`, sharing one projection mapper with the backfiller. `selection()` skips a connection row whose URL already has a pool card, so nothing is listed twice. The per-platform `POST /platforms/{platform}/events` verb is NOT repointed — see the manifest's "Residual".
