...
**Assessment:** with a 60s TTL the orphan population is bounded by *edit rate × 60s*, not by total sites, so this scales fine. **No change.** It is listed only because it is the mechanism by which §2.1 becomes dangerous if the TTL is ever lengthened or the policy is `noeviction`.
