...
- [ ] **TECH-5 · P0** — Secret rotation runbook → `docs/runbooks/secret-rotation.md`
    ...
    - **Google Maps key note:** the `GOOGLE_MAPS_API_KEY` (returned by the authenticated `GET /api/config/integrations` since audit public-surface/SEC-1) is safe only while its GCP **HTTP-referrer restriction** (`*.partna.au/*`) holds. Re-verify that restriction in the Google Cloud Console as part of any rotation **and on every fresh-environment deploy** — dropping it on a new project is the highest-risk moment.
