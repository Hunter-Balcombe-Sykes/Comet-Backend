...
- **Skeleton pattern missing on pre-create checks.** Before a resource row exists, build a skeleton instance (e.g., `new SiteMedia(['user_id' => $user->id, 'usage' => 'content'])`) and call `authorizeForUser($user, 'create', $skeleton)` rather than skipping authz entirely.
