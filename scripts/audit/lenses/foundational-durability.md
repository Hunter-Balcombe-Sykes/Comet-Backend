...
- **Platform integration subsystem (converged 2026-07-05, FOUND-24):** `app/Http/Controllers/Api/Platforms/` (~20 bespoke controllers, down from a per-platform-fan-out pattern) and `app/Services/Platforms/` (services + `Strategies/{Connect,Highlights,Fetch,Refresh,Detect}/`). The canonical "add a platform" path is now ONE descriptor registration in `app/Providers/PlatformRegistryServiceProvider.php` (a `ConnectStrategy`, an optional `HighlightsStrategy` for picker platforms, and a route shape) — `tests/Feature/Architecture/PlatformControllerConvergenceTest.php` guards against regressing to a new bespoke controller or hand-written route group. Central question: has any platform added since the convergence reintroduced a per-platform controller/route/`match()` arm instead of a descriptor + strategy, and are the shared concerns (rate-limiting, capability gating, validation, connection persistence, refresh scheduling, error shaping) still owned centrally rather than re-forked per platform?
