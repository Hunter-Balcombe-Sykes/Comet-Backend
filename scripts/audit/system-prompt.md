...
- **Architecture system:** `site.sites.architecture_id` (TEXT CHECK `= 'one'`; single-architecture, renamed from `skeleton_id`) + `site.design_kits` (1:1, nullable columns, defaults live in code, not the DB). `site.themes` and `settings.design.*` are removed — code writing to either is a finding. The transitional `skeletonId` wire-key alias in `IndividualProfileResource` is intentional legacy — do NOT flag it. (The `site.public_site_payload` view and its `skeleton_id` JSONB key were DROPPED 2026-09-04 with `GET /api/public/site` and `PublicSiteController`; any surviving reference to that view is a real finding, not suppressed legacy.)
- **Outbound URL fetches** (platform connectors, design auto-grabbers) must go through `SafeUrlFetcher` (`app/Services/Http/SafeUrlFetcher`, SSRF host allowlist). Raw `Http::get($userUrl)` on user-supplied URLs is a finding.
- **Platform integrations:** the link / curated-highlights archetype (music/video/social platforms) converged onto a single descriptor-driven path — a `PlatformDescriptor` registered in `app/Providers/PlatformRegistryServiceProvider.php` pairs a `ConnectStrategy` (plus a `HighlightsStrategy` for picker platforms) with route/resource wiring, served by `GenericPlatformController`. ~20 bespoke controllers remain for platforms with a genuine reason (pickers, async connect, cross-platform reads), allowlisted in `PlatformControllerConvergenceTest`; a NEW bespoke controller or hand-rolled route group for a platform that fits the generic shape is a finding.
