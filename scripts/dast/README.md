...
3. **The IDOR probe surfaces** (`zap-active.sh`'s `requestor` job) — each probe hardcodes a route (`/api/customers/{id}`, `/api/enquiries/{id}/read`) and its expected status. If one of those routes is renamed or removed, the probe keeps "passing" because a missing route also 404s — the paired `control-*` request is what catches it, so never drop a control to quieten a failure.
