...
## Follow-up (security review, 2026-07-11) — fail CLOSED on a blank/absent email claim
Review found the binding fell OPEN on tokens with no email claim: the bootstrap route requires `supabase.jwt` but NOT `require.email_verified`, so an anonymous/phone-only Supabase token (valid `sub`, no `email` claim) reached bootstrap and — under the original "empty claim → keep body" fallback (plan §1) — trusted the attacker-controlled body email, reopening the invite email-match spoof + email squat. **Superseding fix:** `BootstrapController::bootstrap()` now rejects with `422 EMAIL_VERIFICATION_REQUIRED` when the verified claim email is blank/absent, before the invite-match / uniqueness / create paths. `prepareForValidation` still binds the verified claim on the happy path; normal email/OAuth signup always carries the claim so it's unaffected. Existing direct-controller bootstrap tests (waitlist-gate ×2, divert/disabled ×3) now carry a verified claim email so they still exercise their target behaviour. New tests: blank-claim + body-spoofs-invite + valid token → 422, invite stays `invited`, no User minted; blank-claim normal signup → 422; non-admin staff → policy denial (`AuthorizationException`) on `store()`.
