...
_Reconciled 2026-07-25 from `audits/archive/workers/2026-07-23-worker-async-review-TRIAGE.md` (this sweep is that review's **R3**). `SCALE-1` is `[~]` — relocated, not done._
...
- [x] **#JOB-1** · P1 — `SendEnquiryConfirmationJob` stamps the idempotency flag before the mail send, masking permanent failures
...
- [x] **#JOB-2** · P1 — `SendSubscriptionConfirmationJob` same stamp-before-send masking pattern
...
- [x] **#JOB-3** · P1 — `SendEnquiryNotificationJob` same stamp-before-send masking pattern
...
- [x] **#OBS-1** · P1 — `SourceGenerationException` swallowed in `GeneratePreAccountSiteJob.handle()` — build failure invisible to Nightwatch
...
- [x] **#JOB-4** · P1 — `SendAccountDeletionRequestMailJob` stamp-before-send pattern strands a user's GDPR deletion request with no recovery path
...
- [x] **#OBS-2** · P2 — `ProcessImageVariantsJob` swallows cache-purge dispatch failures with no `report()`
...
- [x] **#OBS-3** · P2 — `MenuFetchJob` swallows scan-reapply failures with no `report()` — enrichment loss invisible
...
- [~] **#SCALE-1** · P2 — Analytics ingest shares a single 2-process Horizon lane with 10 other queues, list-ordered behind all of them _(RELOCATED 2026-07-24 — not done. Graduated to its own unit; needs a capacity decision, not a routine fix. Tracked in the worker-async review TRIAGE at `audits/archive/workers/2026-07-23-worker-async-review-TRIAGE.md`.)_
...
- [x] **#OBS-4** · P2 — `SourceGenerationException` swallowed in `ApproveEarlyAccessBuildJob.handle()` — approval scrape failure invisible to Nightwatch
...
- [x] **#CACHE-1** · P2 — `ReconcilePlatformTakedownJob`'s per-model save loop can flood the `cloudflare` queue and delay unrelated users' cache purges
...
- [x] **#OBS-5** · P2 — Lock-contention timeouts in `GoogleBusinessEnrichJob` and `EnrichLinkCardJob` log warnings but never call `report()` — sustained contention invisible
...
- [x] **#OBS-6** · P2 — Eight platform/pre-account jobs have no `failed()` callback — terminal failures land silently in `failed_jobs` with no Nightwatch signal
...
- [x] **#SCALE-2** · P2 — `SendStaffBroadcastEmailToSubscriberJob` has no email-provider rate limiting — a large broadcast can exceed Resend/Postmark per-second caps
...
- [x] **#SCALE-3** · P2 — `ProcessVideoVariantsJob` declares three backoff gaps but `$tries = 2` only ever consumes the first
...
- [x] **#CCH-1** · P3 — `GuardsMediaProcessing` uses raw `Redis::set`/`Redis::del` instead of the dedicated `cache_locks` connection
...
- [x] **#JOB-5** · P3 — `SyncCustomerMarketingOptInJob` missing `ShouldBeUnique` on a concurrency-sensitive cache-column write
...
- [x] **#OBS-7** · P3 — `SendEnquiryNotificationJob` hardcodes its queue name instead of reading it from config
