...
- Standalone: 21 / 26 complete (PRIV-4, SEC-1, PRIV-2, DINT-1, PRIV-7, PRIV-8, DINT-6, LIFE-1c, WHK-3 + schema batch DINT-2,3,7,8,10,11 SCHEMA-2,7 + MIG-2/5/6 resolved-by-convention + P3-11 obsolete) · 5 deferred (CCH-2, PRIV-3, WHK-4, EDGE-10, GS-8) · schema migrations APPLIED to dev 2026-06-30 (PRIV-2 grant `20260624000000` + schema batch `20260624010000/010100/010200` + WHK-3 `20260625000000`); verified via list_migrations + live object check
...
- [ ] **#WHK-4** · P3 — ⏸ **DEFERRED (Josh, 2026-06-25):** replay command intentionally not built. WHK-3 was scoped trail-only and deliberately does NOT persist the auth `token_hash` (PII posture), so true replay is impossible without first re-adding token retention. Revisit only if auth-email replay becomes an operational need — would require a `token_hash` column, a `'replayed'` status, and the command — → `audits/archive/loose-may-2026/audit-2026-05-25-email-change-flow.md`
...
- [ ] **#EDGE-10** · P2 — ⏸ **DEFERRED (Josh, 2026-06-25):** no staging Worker deployed yet — the placeholder KV namespace IDs are inert until a staging environment is provisioned. Revisit when staging stands up (paste the real namespace IDs into `cloudflare-worker/wrangler.toml:50-53`) — `cloudflare-worker/wrangler.toml:50-53` → `audits/archive/codebase-full-sweep-2026-06-13/audit-2026-06-13-edge-worker.md`
...
- [ ] **#GS-8** · P3 — ⏸ **DEFERRED (Josh, 2026-06-25):** one-time Cloudflare dashboard action, not a repo change. ⚠️ **UNSAFE AS ORIGINALLY WRITTEN — cross-tenant cache-poisoning (verified 2026-06-30):** the finding says "cache rule on `/api/public/*`", but (1) the only path that emits public cache headers is **`api/public/site-by-slug`** (the `AddPublicCacheHeaders::CACHEABLE_PATH_PREFIXES` allowlist is exactly that one prefix), and (2) that route (`PublicSiteController::showByHeader`) resolves the tenant **purely from the `X-Site-Subdomain` request header** — the URL `https://api.partna.au/api/public/site-by-slug` is byte-identical for every tenant. The middleware sends `Vary: X-Site-Subdomain`, but **Cloudflare ignores `Vary` except `Accept-Encoding`** and keys on URL only, so a naive cache rule would cache the first tenant's profile and serve it to ALL tenants (data leak, not just staleness). **Safe enable requires ALL of:** (a) scope the rule to `api/public/site-by-slug` exactly, NOT `/api/public/*`; (b) a **Custom Cache Key that includes the `X-Site-Subdomain` header** — verify plan supports headers-in-cache-key (historically Enterprise); without it, do NOT enable; (c) Edge TTL = **respect origin headers** (middleware already sets `s-maxage=900`), never a flat forced TTL. Alternative: re-architect `site-by-slug` to carry the tenant in the URL (`?slug=`/host) so edge caching is cache-key-safe for free — bigger than this P3 warrants pre-beta. **Cannot be run from the repo env** (no Cloudflare CLI/creds locally; the project `cloud` CLI is Laravel Cloud, not Cloudflare) — `app/Http/Middleware/AddPublicCacheHeaders.php` → `audits/archive/loose-may-2026/audit-2026-05-07-caching-foundation.md`
