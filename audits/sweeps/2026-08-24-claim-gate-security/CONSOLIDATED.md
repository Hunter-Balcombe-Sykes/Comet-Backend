...
- [x] **#SEM-2** · P2 — `isOutreach()` reverts a staff-built site to first-come claiming if the creating staff row is later hard-deleted
    ...
    - **Premise correction (2026-08-25, ManyChat claim links):** the Technical note below states `built_via === 'staff'` "can only originate from an actual staff-authenticated write" — that is no longer true. `POST /api/internal/webhooks/manychat/builds` (static-secret gated, no staff auth) now also sets `built_via = VIA_STAFF`. The conclusion this finding fixed still holds — checking `built_via` in addition to `built_by_staff_id` still correctly keeps the build outreach-classified and gated — the fix is if anything now load-bearing for a second, non-staff write path, and misclassification still fails safe (toward more gating, not less). Only the stated premise is stale.
