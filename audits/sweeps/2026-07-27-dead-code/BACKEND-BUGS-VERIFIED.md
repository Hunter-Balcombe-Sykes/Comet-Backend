...
- [ ] **#FFLAG-1** · P0 — `POST /staff/feature-flags/{key}/overrides` 500s on every call: the response re-read filters on `brand_id`, a column dropped from `core.feature_flag_overrides` two months ago
...
- [ ] **#TESTFX-1** · P1 — Two hand-maintained SQLite stand-ins for the same table have silently diverged, and the one the tests use is the wrong one — while its docblock claims they mirror
