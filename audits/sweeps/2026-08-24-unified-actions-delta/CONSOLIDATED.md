...
- [x] **#API-7** · P3 — Public pool hydration runs a dashboard-only duplicate-detection query on every request
    - **FIXED 2026-08-26.** `itemPayloads()` now takes `$withDuplicateCandidates`; `resolve()` (the dashboard — `PoolController::show`, `PoolItemCreateController::store`, both owner-authenticated) passes `true`, and the `hydrateItems()` seam defaults it **OFF**. Opt-in, not opt-out: a caller that forgets the flag loses a dashboard chip (visible), whereas forgetting to pass `false` would silently put a query back on the hot path (not visible).
        **NOT a wire change — deliberately.** `duplicateCandidates` stays in the payload with `[]`, so `ITEM_KEYS` / `PoolWireShapeTest` are untouched and no `docs/wire-changes/` manifest is needed. Dropping the KEY rather than the QUERY would have been a wire change.
        **Sizing, measured rather than assumed — the headline "every request" is right for one caller and wrong for another.** `PoolWire::forSite()` has three consumers: the public profile payload (behind `rememberLocked` at `partna.public_profile.cache_ttl_seconds` = 60s, so **per cache miss**, not per request), `GET /site/actions` via `ActionCandidates` (**uncached — genuinely per request**, and the strongest win), and `ComputeContentPopularityScores` (per site per run).
        **Row volume was never the cost.** `EXPLAIN (ANALYZE, BUFFERS)` on dev: the partial index `idx_identity_candidates_open (user_id) WHERE dismissed_at IS NULL` makes the candidate side an index scan returning 0 rows, so both `content.items` joins report `never executed` — 1.85 ms total, almost all of it the synthetic top-user CTE. Dev holds **1 candidate row, 0 open**; prod has no `content` schema at all. The win is **one guaranteed DB round trip per hydration**, the same unit the 2026-08-24 batching refactor was denominated in (244 queries → ~60 took `/site/actions` from 58.8 s to single digits ⇒ ~240 ms/round trip on that link). It also stops the cost scaling with candidate accumulation later.
        Tests: `tests/Feature/Content/PoolWireDuplicateCandidateGateTest.php` (3 cases, bidirectional — public path must NOT query, dashboard path MUST, and the payload shape is unchanged). Mutation-proved: un-gating the query fails `expect($hits)->toBe([])`; passing `false` from `resolve()` fails `toHaveCount(1)`; dropping the `#SEC-5` right-side tenant scope still fails the cross-tenant leak test.
        **`#SEC-5`'s guard was at risk of being defanged and was kept sharp.** `IdentityCandidateTenantScopingTest` goes through `hydrateItems()` and asserts `duplicateCandidates` is `[]`; with the read gated off by default that would have passed for the wrong reason. Both cases now pass `withDuplicateCandidates: true` explicitly, so the test is about TENANCY, not about the gate.
        **One collateral fix.** `PoolDegradedBuildTest`'s anonymous `PoolResolver` subclass overrode `hydrateItems()` with the old 2-arg signature. PHP fatals at CLASS-DECLARATION time when a child drops a parent's optional param, so this surfaced as pest exiting 2 with **no output at all** — not a test failure. Signature now tracks the parent, exactly as SCALE-2 had to do for `assemble()`'s `$withLibrary`.
        Verified: full suite **9256 passed, 3 skipped, 0 failed**; pint + phpstan clean.

        **Duplicate of `SCALE-9`** in `audits/sweeps/2026-08-24-unified-actions-remainder/CONSOLIDATED.md` — one defect, fixed once, ticked in both. NOT the overnight sweep's `#SCALE-9` (slug `ensureCurrent()` N+1), which is a different defect closed 2026-08-25 by `adaeb9ae2`.
