...
- Standalone: 21 / 26 complete (PRIV-4, SEC-1, PRIV-2, DINT-1, PRIV-7, PRIV-8, DINT-6, LIFE-1c, WHK-3 + schema batch DINT-2,3,7,8,10,11 SCHEMA-2,7 + MIG-2/5/6 resolved-by-convention + P3-11 obsolete) · 5 deferred (CCH-2, PRIV-3, WHK-4, EDGE-10, GS-8) · schema migrations pending `supabase db push` (PRIV-2 grant + schema batch + WHK-3)
...
- [ ] **#GS-8** · P3 — ⏸ **DEFERRED (Josh, 2026-06-25):** one-time Cloudflare dashboard action (cache rule on `/api/public/*`), not a repo change — response headers (`AddPublicCacheHeaders`) are already correct. Tracked as an infra TODO; add the CF cache rule when prioritised — `app/Http/Middleware/AddPublicCacheHeaders.php` → `audits/archive/loose-may-2026/audit-2026-05-07-caching-foundation.md`
