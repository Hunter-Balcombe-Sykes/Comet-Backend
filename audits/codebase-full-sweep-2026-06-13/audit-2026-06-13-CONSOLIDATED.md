...
- [ ] **S3 complete** — 🚧 _prepared 2026-06-15 (Lane C): migrations written + reviewed (`20260615000000_harden_smart_links.sql` DDL + `20260615000001_smart_links_canonical_idx.sql` CONCURRENTLY index) + on development (ab71f134); guard green. ⛔ `supabase db push` BLOCKED on pre-existing dev migration drift — 6 versions (20260611151519, 20260612105733, 20260613054302/8, 20260613054413, 20260613084455) are applied on dev but absent from repo AND git history (applied directly, likely by primary dev). Needs Josh's reconcile decision (`migration repair --status reverted` to discard from tracking vs `db pull` to adopt) BEFORE these can push. SCHEMA-5/DINT-5 CHECKs deferred; FORCE RLS → S5._
- Models: plan=opus · impl=sonnet · review=opus
- Findings:
