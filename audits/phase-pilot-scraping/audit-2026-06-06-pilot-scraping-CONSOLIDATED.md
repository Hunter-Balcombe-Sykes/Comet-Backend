...
- [x] **#CONS-38** · P3 · Effort: L — Platform controllers return hand-assembled arrays without Resource classes — no canonical field allowlist per platform
    - **Status (2026-06-09):** DONE. Public half shipped with CONS-11; authenticated half shipped here. The 8 platform controllers' selection responses now flow through 9 new array-backed Resources under `app/Http/Resources/Platforms/`: `LinkConnectionResource` (Facebook + TikTok), `TileConnectionResource` base + `Youtube`/`AppleMusic`/`ApplePodcast` subclasses, `EventbriteConnectionResource`, `InstagramConnectionResource`, `ShopifyBrandResource` (single + collection), `FreshaSelectionResource`. Each `toArray()` is the canonical per-platform top-level allowlist; nested scraper items (latest tile, highlights, products, services, events, images) pass through verbatim. These are a separate ARRAY-backed family wrapping the bare selection payload — not the model-backed public Resource (so no `$this->when()` reuse; the two contracts differ by design). Byte-identical dashboard contract with ONE documented deviation: Instagram's internal `_folder` is now stripped from the authenticated responses too (it was leaking; the public endpoint already stripped it; the stored row keeps `_folder` for the cleanup observer). Transient pickers (`recent`/`posts`/`brandProducts`/`team`/`employee-services`), scraper-list envelopes, the Shopify compat-flat `selection`, the Fresha menu shape, and IG control responses stay inline by design. Proven by `tests/Feature/Platforms/PlatformResourceContractTest.php` (per-platform `assertExactJson` characterization + unknown-key-strip assertions); full suite green (2028 passed). Note: CONS-21 R2 cleanup still leaves ONE documented residual — a manual→automatic mode switch orphans the manual folder (the async placeholder null-write erases the folder reference before the new folder is known); rare, low-severity, like the pre-`_folder` legacy folders.
