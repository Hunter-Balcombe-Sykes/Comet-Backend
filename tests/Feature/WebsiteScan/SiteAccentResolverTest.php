...
function seedMediaRow(string $siteId, array $overrides = []): string
{
    $id = (string) Str::uuid();
...
it('falls back to the gallery palette when theme-color, logo, and favicon all fail', function () {
    ...
    seedMediaRow($siteId, ['pool' => SiteMedia::POOL_GALLERY, 'dominant_color' => '#ab3516']);
...
it('prefers the favicon over gallery when no logo palette exists', function () {
    $siteId = (string) Str::uuid();
    seedMediaRow($siteId, ['pool' => SiteMedia::POOL_GALLERY, 'dominant_color' => '#334455']);
...
it('skips a non-qualifying candidate and falls through to the next tier', function () {
    $siteId = (string) Str::uuid();
    seedMediaRow($siteId, ['pool' => SiteMedia::POOL_GALLERY, 'dominant_color' => '#e0491f']);
...
it('never crosses tenants — a candidate on another site is never picked', function () {
    ...
    seedMediaRow($otherSiteId, ['pool' => SiteMedia::POOL_GALLERY, 'dominant_color' => '#e0491f']);
