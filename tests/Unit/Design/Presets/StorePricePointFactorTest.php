...
it('classifies a mid median to the neutral recipe', function () {
    ...
    expect($out['space_regular'])->toBe('0.625rem')
...
it('holds the prior band when a drift crosses the boundary by less than 15%', function () {
    // Prior = casual (0.5rem). New median 44 is just over CASUAL_MAX (40) into
    // mid, but only by 10% — inside the dead-band, so the band must HOLD casual.
    $out = (new StorePricePointFactor)->detect(spEvidence([40, 42, 45, 49], priorSpace: '0.5rem'));

    expect($out['space_regular'])->toBe('0.5rem'); // still casual, no flicker
});
...
it('requires two consecutive decisive crosses before flipping the band', function () {
    ...
    $first = $factor->detect(spEvidence([80, 85, 95, 100], priorSpace: '0.5rem'));
    expect($first['space_regular'])->toBe('0.5rem'); // held — one cross isn't enough

    // Second consecutive decisive cross -> commit the flip to mid.
    $second = $factor->detect(spEvidence([80, 85, 95, 100], priorSpace: '0.5rem'));
    expect($second['space_regular'])->toBe('0.625rem'); // flipped to mid
});
...
it('resets the streak when a drift falls back inside the prior band', function () {
    ...
    $factor->detect(spEvidence([80, 85, 95, 100], priorSpace: '0.5rem'));

    // Then the catalog drops back to casual — streak must reset, band stays casual.
    $back = $factor->detect(spEvidence([15, 20, 25, 30], priorSpace: '0.5rem'));
    expect($back['space_regular'])->toBe('0.5rem');

    // A fresh decisive cross now needs TWO more reads, proving the reset:
    $one = $factor->detect(spEvidence([80, 85, 95, 100], priorSpace: '0.5rem'));
    expect($one['space_regular'])->toBe('0.5rem'); // held again — streak restarted
});
