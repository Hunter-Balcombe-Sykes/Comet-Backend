...
it('has the required queue-hygiene properties and unique id', function () {
    $job = new EnrichLinkCardJob('u', 'online-ordering', 'link-abc', 'https://x.com', 'uber_eats.order');
    expect($job->tries)->toBe(3)
        ->and($job->backoff)->toBe([30, 120, 300])
        ->and($job->timeout)->toBe(60)
        ->and($job->uniqueId())->toBe('online-ordering:link-abc');
});
...
it('upgrades display fields from the snapshot and marks the row ok', function () {
    ...
    (new EnrichLinkCardJob($user->id, 'online-ordering', 'link-abc', 'https://x.com', 'uber_eats.order'))->handle(app(LinkCardScraper::class));
...
it('no-ops when the row is gone', function () {
    ...
    (new EnrichLinkCardJob($user->id, 'online-ordering', 'missing', 'https://x.com', 'uber_eats.order'))->handle(app(LinkCardScraper::class));
});
